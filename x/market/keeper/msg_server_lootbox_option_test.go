...
func TestLootboxOptionMsgServerDelete(t *testing.T) {
	creator := "A"

	for _, tc := range []struct {
		desc    string
		request *types.MsgDeleteLootboxOption
		err     error
	}{
		{
			desc: "Completed",
			request: &types.MsgDeleteLootboxOption{Creator: creator,
				Name: strconv.Itoa(0),
			},
		},
		{
			desc: "Unauthorized",
			request: &types.MsgDeleteLootboxOption{Creator: "B",
				Name: strconv.Itoa(0),
			},
			err: sdkerrors.ErrUnauthorized,
		},
		...
		t.Run(tc.desc, func(t *testing.T) {
			...
			_, err := srv.CreateLootboxOption(wctx, &types.MsgCreateLootboxOption{Creator: creator,
				Name: strconv.Itoa(0),
			})
			require.NoError(t, err)
			_, err = srv.DeleteLootboxOption(wctx, tc.request)
			if tc.err != nil {
				require.ErrorIs(t, err, tc.err)
			} else {
				require.NoError(t, err)
				_, found := k.GetLootboxOption(ctx,
					tc.request.Name,
				)
